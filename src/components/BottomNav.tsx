...
const tabs: { id: TabType; label: string; icon: string }[] = [
  ...
  { id: 'rewards', label: 'Rewards', icon: 'fa-gem' },
...
export default function BottomNav({ activeTab, onTabChange, cartCount }: BottomNavProps) {
  ...
  return (
    <nav className="shrink-0 bg-black/95 backdrop-blur-xl border-t border-white/5 px-1 pb-2 pt-1">
          ...
          return (
            ...
            >
              <div className="relative">
                ...
                <i className={`fa-solid ${tab.icon} text-base relative z-10`} />
              ...
              <span className={`text-[9px] mt-0.5 font-medium transition-all ${isActive ? 'opacity-100' : 'opacity-60'}`}>
                {tab.label}
              </span>
              {isActive && (
                <div className="absolute -bottom-0.5 w-1 h-1 rounded-full bg-violet-400" />
              )}
            </button>
          );
        })}
